module EXAM.LTS where
import Data.List 

{-
An overview of util functions for Model Based Systems questions in the exam. Combination of LTS.hs
and results of the Labs.
-}
...
-- For Exercise 4 (traversing list of labels and match something)


-- Checks whether the trace is correct and contains the matched labels (as output)
-- matchLabels gives the list of labels that you want to look for
-- impl gives the Implementation as an LTS (can be quickly changed for IOLTS
-- if needed)
matchOutputs :: [Label] -> [Label] -> LTS -> Bool
matchOutputs trace matchLabels impl = matchOutputs' trace initial lt matchLabels
    where
        (_, _, lt, initial) = impl

matchOutputs':: [Label] -> State -> [LabeledTransition] -> [Label] -> Bool
matchOutputs' [last] _ _ matchLabels = last `elem` matchLabels
matchOutputs' (next_l:trace) state lt matchLabels
    | next_l `elem` matchLabels = True
    | otherwise = any (\(s', l) -> matchOutputs' trace s' lt matchLabels) nextStates
  where
...
-- Gets all traces from an LTS
traces :: LTS -> [Trace] -- [[Label]]
traces (q, l, lt, q0) = nub $ map snd (traces' lt [([q0],[])])

-- All traces from IOLTS
ioTraces :: IOLTS -> [Trace]
ioTraces (_, _, _, [], _) = []
ioTraces (_, _, _, lt, q_0) = nub $ map snd (traces' lt [([q_0],[])])

-- Following was used from:
-- https://github.com/AndreasElgaard/SSVT-Exercises/blob/main/Lab%204/Exercise4.hs
-- Our own implementation had some flaws, which were annoying in practicing exams
