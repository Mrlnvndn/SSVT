...
countSurvivors numberOfMutants listOfProperties functionUnderTest = do
  mutations <- getMutants numberOfMutants --creates IO list of mutators
  results <- forM mutations $ \mutation ->
    do mutate' mutation listOfProperties functionUnderTest
      <$> generateRandomInteger -- Three above lines calls mutate' for every single value from mutations list and list of properties
  determineSurvivingMutants results

generateRandomInteger :: IO Integer
generateRandomInteger = randomRIO (1, 100)

-- gets n times a random value from mutators list
...
properties' = [prop_moduloIsZero', prop_sumIsTriangleNumberTimesInput', prop_firstElementIsInput', prop_tenElements']
...
main :: IO ()
main = do
  survivors <- countSurvivors 10000 properties' multiplicationTable
  print survivors