...
import Utils (TypeFut, PropMap, MutationMap, printResMap, mapKeys)
import FitSpec (properties', multiplicationTable, propMap)
import Mutation (mutators)
import Exercise3 (minPropSubset)
...
terminalReport props nMutators nMutations survivors mps = do
    ...
    putStrLn $
        "Used " ++ show nMutators ++ " mutants to create " ++ show nMutations ++ " mutations\n"
        ++
        "There were " ++ show survivors ++ " surviving mutations" ++ sep
    
    putStrLn $ "After eliminating properties that: don't kill any mutants,\n"
