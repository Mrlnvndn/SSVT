...
using Haskell functions. Below we will detail our approach.
...
First we tried to write the statements of the boys down in a logical shortform:
...
## First Solving manually (because we struggled to wrap our head around how it could be concluded)
...
From the table we concluded there is only one boy, who is accused by exaclty 3 people.
Knowing for sure (we can assume the teacher speaks the truth) 3 boys have to be speaking the truth they need
to accuse the correct person, which can therefore only be Jack.

## Converting into Haskell code
...
This makes it easier to calculate the relationships (especially Jacks, Arnold and Carls statements).
Ultimately this gives the same result: Jack is guilty.
...
-- Helper function for every type of accusation, which is specified below
...
accuses Peter _ = False --otherwise not accused

-- Oneliner based on Jacks statement (Opposite of sections above)
